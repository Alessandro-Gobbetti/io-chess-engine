#pragma once
/**
 * TimeManager - Sophisticated time allocation for chess engine
 *
 * Features:
...
public:
   ...
   * @param allocation       Previously calculated allocation
   * @return true if should stop searching
   */
  bool shouldStop(int64_t elapsed, int depth, int bestMoveChanges,
                  int scoreDrops, const TimeAllocation &allocation) const;

  /**
   * Get extension factor based on search instability.
   * Called during iterative deepening to adjust time limits.
   */
